...
class ErrorAdjustment:
    ...
    def set_error_and_radian(self, error_x, error_y, theta,fps):
        ...
        self.velocity = (self.error - self.prev_error) / (1/fps)
        self.acceleration = (self.velocity - self.prev_velocity) / fps
        self.integral += (self.error + self.prev_error) / 2 * (1/fps)
        self.velocity_theta = (theta - self.prev_theta) / (1/fps)
        self.integral_theta += (theta + self.prev_theta) / 2 * (1/fps)
        ...
        axis_z = (AXIS_P * np.cos(theta) + AXIS_D * np.sin(self.velocity_theta) 
                                        + AXIS_I * np.sin(self.integral_theta))
    
        axis_control_z = -PARAMETER_A * axis_z
        self.speeds[0] = axis_control_z
        self.speeds[1] = axis_control_z
        self.speeds[2] = (p_control_y + d_control_y + k_control_y + i_control_y) 
        self.speeds[3] = - (p_control_y + d_control_y + k_control_y + i_control_y) 
            ...
            if MIN_AREA < area < MAX_AREA:
                ...
                if centroid[0] != -1 and centroid[1] != -1:
                    ...
                    if stable_detector.is_stable():
                        ...
                        last_control_time = error_speed_controller.set_error_and_radian(error_x, error_y, radian,fps)
