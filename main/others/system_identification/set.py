...
def motor_control_thread():
    ...
    try:
        with open("random_data.csv", "r") as csvfile:
            ...
            for row in reader:
                speed_x1, speed_x2, speed_y1, speed_y2 = [int(value) * 10 for value in row]
...
class MouseTracker:
    def __init__(self, device_paths, k_angle):
        ...
        self.file_lock = threading.Lock()  # ファイルロックを初期化
    ...
    def read_mouse(self, device_path, mouse_index):
        ...
        for event in dev.read_loop():
            ...
            if event.type == ecodes.EV_REL:
                ...
                self.mouse_positions[mouse_index] = (get_point_x, get_point_y)
    ...
    def calculate_average(self):
        ...
        while motor_control_active:
            ...
            self.prev_radians = (radian_x, radian_y)
            self.prev_angular_velocity = (angular_velocity_x, angular_velocity_y)
            self.prev_time = current_time

            with data_lock:
                speed_x_diff = current_speeds[1] - current_speeds[0]
                speed_y_diff = current_speeds[2] - current_speeds[3]

            with self.file_lock:
                if self.data_file:
                    # 指定された形式でデータを保存
                    self.data_file.write(f"{angular_accel_x},0,{angular_velocity_x},0,{speed_x_diff}\n")
                    self.data_file.write(f"0,{angular_accel_y},0,{angular_velocity_y},{speed_y_diff}\n")
